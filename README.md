...
Ouvrir `index.html` dans un navigateur, ou servir le dossier via GitHub Pages. Régler les 3 valeurs avec les barillets (molette, tactile, clic ou flèches du clavier) — **le solde se recalcule en direct** à chaque cran, sans bouton. Au départ (et sur Réinitialiser) : 30 jours / 0 heure / 0 %, comme en début d'abonnement. Les dernières valeurs sont mémorisées (localStorage) et restaurées à la visite suivante :

1. **Jours restants** dans le cycle (0–30, boucle : passer sous 0 revient à 30 et recommence un nouveau cycle — heures et consommation remises à 0 ; baisser les jours cale les heures sur 23)
...
- Équilibre théorique = temps restant (`720 × (1 − consommation / 100)` converti en jours + heures) pour lequel la consommation correspondrait exactement au temps écoulé, avec la date/heure locale de cet équilibre et l'échéance (`dans X` si futur, `il y a X` si passé — calculs en timestamps, affichage local, aucun parsing de chaîne).
